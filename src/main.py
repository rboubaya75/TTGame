...
POWERUP_DURATION = 5000  # Duration of the power-up effect in milliseconds
...
class Paddle:
    ...
    def update(self):
        if self.powerup_end_time and pygame.time.get_ticks() > self.powerup_end_time:
            self.reset_powerup()
...
class Game:
    ...
    def update_powerups(self):
        ...
        if self.powerup_timer >= FPS * 10:  # Every 10 seconds
            self.spawn_powerup()
            self.powerup_timer = 0
